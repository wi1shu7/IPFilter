...
from bullets import _BaseRequestClass
"""
测试模块，示例，可自行编写其他脚本进行使用
"""
class test(_BaseRequestClass):
    """
    该类用于为发送封禁请求提供模板
    扫描模块时只会扫描该类的子类
    ...
    def request(self, ip: str, data: dict) -> Dict[Union[str, int], Union[str, int, dict, list]]:
        ...
        _test = {"ip": ip, "data": data}

        requests.post("http://127.0.0.1:60054", json=_test, headers={'content-type': 'application/json'})
        return {"status": 1,
                "message": "发送成功，请前往http://127.0.0.1:60054查看",
                "error": "发送失败",
                "data": {"test": "test"}
                }
